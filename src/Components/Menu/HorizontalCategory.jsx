    ...
    const categories = [
      ...
      return (
        <div className="h-category-container">
          <div className="h-category-row text-focus-in">
            {categories.map((category, index) => (
              <div className="h-category-item" key={index}>
                <Link
                  to="/SubCategory"
                  style={{ textDecoration: "none", color: "inherit" }}
                >
                  <img src={category.image} alt={category.name} style={{ width: "50px", height: "50px", borderRadius: "50%" }} />
                  <p>{category.name}</p>
                </Link>
              </div>
            ))}
